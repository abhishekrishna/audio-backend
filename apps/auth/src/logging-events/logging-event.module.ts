...
import { LoggingEvent, LoggingEventSchema } from './entities/logging-event.entity';
import { LoggingEventController } from './logging-event.controller';
import { LoggingEventService } from './logging-event.service';

@Module({
  imports: [MongooseModule.forFeature([{ name: LoggingEvent.name, schema: LoggingEventSchema }])],
